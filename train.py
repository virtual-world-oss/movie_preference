...
def adjust_lr(opti,epoch):
    lr = cofig['lr'] * (0.3 ** (epoch // 20))
    if lr >= 0.00001:
        for para in opti.param_groups:
            para['lr'] = lr

def train(epoch,best_acc,best_epoch):
    #train
    t = time.time()
    model.train()
    optimizer.zero_grad()
    ...
    if acc_val > best_acc:
        best_acc = acc_val
        best_epoch = epoch
        torch.save(model, f'Model\ModelSavebest\GCNModel_train_acc_{acc_train}_val_acc_{acc_val}_epoch_{epoch}.pth')
        torch.save(model.state_dict(), f'Model\ModelSavebest\state_GCNModel_train_acc_{acc_train}_val_acc_{acc_val}_epoch_{epoch}.pth')
    if epoch % cofig['epoch_save'] == 0:
        torch.save(model, f'Model\ModelSaveauto\GCNModel_train_acc_{acc_train}_val_acc_{acc_val}_epoch_{epoch}.pth')
        torch.save(model.state_dict(), f'Model\ModelSaveauto\state_GCNModel_train_acc_{acc_train}_val_acc_{acc_val}_epoch_{epoch}.pth')
...
for epoch in range(cofig['epochs']):
    ...
    best_acc,best_epoch = train(epoch,best_acc,best_epoch)
